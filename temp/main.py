...
class GenericAssistant(IAssistant):
    ...
    def train_model(self):
        ...
        # sgd = tf.keras.optimizer.SGD(learning_rate=0.01, momentum=0.9, nesterov=True)
        # self.model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
        self.model.compile(loss='categorical_crossentropy', optimizer=tf.keras.optimizers.SGD(learning_rate=0.01, weight_decay=1e-6, momentum=0.9, nesterov=True), metrics=['accuracy'])
    ...
    def _get_response(self, ints, intents_json, is_method_invocked = False):
        if is_method_invocked:
            result = ints
        else:
            try:
                tag = ints[0]['intent']
